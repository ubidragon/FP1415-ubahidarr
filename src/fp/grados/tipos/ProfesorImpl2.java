...
public class ProfesorImpl2 extends PersonaImpl implements Profesor {
	...
	private Map<Asignatura, Double> m;
	...
	public ProfesorImpl2(String dni, String nombre, String apellidos,
		...
		m = new HashMap<Asignatura, Double>();
	...
	public ArrayList<Asignatura> getAsignaturas() {
		...
		return new ArrayList<Asignatura>(m.keySet());
	}

	public List<Double> getCreditos() {
		// hay que hacer una copia de los creditos
		return new ArrayList<Double>(m.values());
	}
	...
	public Double dedicacionAsignatura(Asignatura asig) {
		...
		Double result = 0.0;
		if (m.containsKey(asig)) {
			result = m.get(asig);// getOrDefault(index, getDedicacionTotal());
									// // esto quiere decir que el indice es
									// el de las
	...
	public void imparteAsignatura(Asignatura asig, Double dedicacion) {
		...
		if (m.containsKey(asig)) {
			...
			m.replace(asig, dedicacion);// esto sirve para cambiar los
										// creditos de la poscion indicada
										// de la lista
		} else {
			m.put(asig, dedicacion);
		}
