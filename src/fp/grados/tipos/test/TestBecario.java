...
public class TestBecario {
	...
	private static void testSetEmailExcepcion() {
		...
		try {
			...
			System.out.println(
					"******************** Se ha capturado la excepción UnsupportedOperationException");
		} catch (Exception e) {
			System.out.println(
	...
	private static void testConstructor1(String dni, String nombre,
		...
		try {
			Becario b = new BecarioImpl(dni, nombre, apellidos, fechaNacimiento,
					email, beca, fechaComienzo);
			mostrarBecario(b);
		} catch (ExcepcionBecarioNoValido e) {
			System.out.println(
					"******************** Se ha capturado la excepción ExcepcionBecarioNoValido");
		} catch (Exception e) {
			System.out.println(
					"******************** Se ha capturado una excepción inesperada.");
		}
	...
	private static void testConstructor2(String dni, String nombre,
			String apellidos, LocalDate fechaNacimiento, String email,
			String codigo, Double cuantiaTotal, Integer duracion, TipoBeca tipo,
			LocalDate fechaComienzo) {

		try {
			Becario b = new BecarioImpl(dni, nombre, apellidos, fechaNacimiento,
					email, codigo, cuantiaTotal, duracion, tipo, fechaComienzo);
			mostrarBecario(b);
		} catch (ExcepcionBecarioNoValido e) {
			System.out.println(
					"******************** Se ha capturado la excepción ExcepcionBecarioNoValido");
	...
	private static void testsetFechaComienzo(Becario b,
			LocalDate fechaComienzo) {
		...
		try {
			System.out.println("La fecha de comienzo antes de la operación es: "
					+ b.getFechaComienzo());
			System.out
					.println("La nueva fecha de comienzo es: " + fechaComienzo);
